# NotIn — Technical Spec (V2)

This is the final spec and single source of truth. It maps every feature to the stack (from the [Vibe-Spec reference](./vibe-spec.md)) and includes a phase plan for implementation. Product requirements are in the [V0 spec](./spec-v0.md).
...
Each phase is a self-contained chunk of work that can be planned, implemented, and verified independently. Detail lives in separate files inside `docs/plans/` — these are outlined only for now and will be fully detailed before implementation begins.

| # | File | Scope |
|---|------|-------|
| 1 | `docs/plans/phase-1-foundation.md` | Project scaffold (Next.js 16, pnpm, Tailwind v4, ESLint), Docker + Postgres, Prisma schema for all models, seed script with SA public holidays and sample data |
| 2 | `docs/plans/phase-2-auth.md` | Google OAuth via Auth.js, `@thoughtlab.studio` domain restriction, auto-create user on first login, RBAC permission map, `proxy.ts` route protection, sign-in page |
| 3 | `docs/plans/phase-3-leave-core.md` | Leave request/cancel/approve/decline API routes, Zod validation, balance checking and deduction, half-day logic, carryover logic, balance adjustment (admin) |
| 4 | `docs/plans/phase-4-dashboard-ui.md` | shadcn/ui setup, app shell (sidebar + bottom nav), employee dashboard, admin dashboard, leave request form, leave history, user management page |
| 5 | `docs/plans/phase-5-calendar-holidays.md` | Team calendar (month view, per-person approved leave), public holiday display and admin management, in-app notifications (bell icon, polling, mark as read) |
| 6 | `docs/plans/phase-6-launch.md` | Google Calendar integration (nice-to-have), CI/CD (GitHub Actions), pre-commit hooks, Vercel + Neon deployment, testing suite, final polish |
